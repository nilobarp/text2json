...
export class Parser extends stream.Transform implements IDataParser {
  public parserOptions: IParserOptions;
  public encoding: string;
  public quote: number;
  public escapedQuotes: RegExp;
  public hasFilters: boolean = false;
  public columnFilters: any[];
  public headersParsed: boolean = false;
  ...
  public text2json(data: any, cb?: doneparsing): any {
    const streaming: boolean = typeof cb === "function" ? false : true;
    ...
    const separator: number = new Buffer(this.parserOptions.separator)[0];
    const newline: number = new Buffer(this.parserOptions.newline)[0];
    const crlf: boolean = this.parserOptions.newline === "\n" ? false : true;
    ...
    dataStream.on("data", (buf: Buffer) => {
      ...
      for (i = 0; i < bufEnd; i++) {
        if (buf[i] === separator || buf[i] === newline) {
          const parsed = this._value(buf, colStart, i, elements);
          elements = parsed.values;
          balancedQuotes = parsed.parsed;
          if (balancedQuotes) {
            colStart = i + 1;
          }
        }
      ...
      if (!balancedQuotes && i === bufEnd && colStart < bufEnd) {
        const err = "Unmatched quotes around " + buf.toString(
          this.encoding, colStart, colStart + 20 > bufEnd ? bufEnd : colStart + 20);
  ...
  private _value(buf: Buffer, start: number, end: number, values: any[]): IParsedValues {
    let balancedQuotes: boolean = true;
    ...
    return { parsed: balancedQuotes, values };
  }
  ...
  private createHash(headers: string[], line: string[], streaming: boolean = false): {} {
    ...
    for (const filter of this.columnFilters) {
      _hash[headers[filter]] = line[filter];
    }
    if (streaming) {
      this.emit("row", _hash);
      _hash = null;
  ...
  private defaultOptions(): IParserOptions {
    return {
      encoding: "utf8",
      filters: { columns: [] },
      hasHeader: false,
  ...
  private mergeOptions(options: IParserOptions): IParserOptions {
    const defaultOpt = this.defaultOptions();
    const opt: IParserOptions = options || defaultOpt;
    options = options || {};
